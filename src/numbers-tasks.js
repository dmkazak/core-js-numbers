...
function getAngleBetweenVectors(/* x1, y1, x2, y2 */) {
  ...
  // const xVectorLength = Math.sqrt(x1 ** 2 + x2 ** 2);
  // const yVectorLength = Math.sqrt(y1 ** 2 + y2 ** 2);

  // const vectorsCartesianProduct = x1 * y1 + x2 * y2;

  // return Math.acos(vectorsCartesianProduct / (xVectorLength * yVectorLength));
}
...
function roundToPowerOfTen(num, pow) {
  const remainder = 10 ** pow;

  return Math.round(num / remainder) * remainder;
}
...
function getFibonacciNumber(index) {
  ...
  return (
    this.getFibonacciNumber(index - 2) + this.getFibonacciNumber(index - 1)
  );
}
...
function getSumOfDigits(num) {
  ...
  let remainder = 1;
  let integerPart = num;
  while (Math.floor(num / remainder) !== 0) {
    integerPart = Math.floor((num % (remainder * 10)) / remainder);
...
function isPowerOfTwo(num) {
  ...
  while (resultNumber < num) {
    resultNumber *= 2;
  }

  return resultNumber === num;
}
